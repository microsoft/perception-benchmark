...
class SpatialTemporalOutputAdapter(nn.Module):
    """Cross-attention adapter for spatial-temporal outputs, like video segments.

    :param num_channels: Number of input channels of the video segment
    :param stride_level: always patch size
    :param spatial_patch_size: Int, the patch size over the full image (of single frame) size.
    ...
    def __init__(
        ...
        enc_embd_dim: Optional[int] = None,
        dec_embd_dim: int = 256,
        depth: int = 0,
        ...
        self.enc_embd_dim = enc_embd_dim
        self.dec_embd_dim = dec_embd_dim
        ...
        if context_tasks is not None:
            self.task_embeddings = nn.ParameterDict(
                {
                    task: nn.Parameter(torch.zeros(1, 1, self.dec_embd_dim))
        ...
        self.mask_token = nn.Parameter(torch.zeros(1, 1, self.dec_embd_dim))

        # position embedding
        self.pos_embd_spatial = nn.Parameter(
            torch.zeros(1, self.grid_size[1] * self.grid_size[2], self.dec_embd_dim)
        )
        self.pos_embd_temporal = nn.Parameter(
            torch.zeros(1, self.grid_size[0], self.dec_embd_dim)
        )
        ...
        if self.use_xattn:
            self.decoder = CrossAttention(
                dim=self.dec_embd_dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
                attn_drop=attn_drop_rate,
                proj_drop=drop_rate,
            )
            self.context_norm = norm_layer(self.dec_embd_dim)
            self.query_norm = norm_layer(self.dec_embd_dim)
            self.out_norm = norm_layer(self.dec_embd_dim)

            mlp_hidden_dim = int(self.dec_embd_dim * mlp_ratio)
            self.mlp = Mlp(
                in_features=self.dec_embd_dim, hidden_features=mlp_hidden_dim
            )
        ...
        self.out_proj = nn.Linear(self.dec_embd_dim, self.out_patch_dim)

        if self.enc_embd_dim is not None:
            self.init(enc_embd_dim=enc_embd_dim)

    def init(self, enc_embd_dim: int = 768):
        """
        ...
        self.proj_context = nn.Linear(self.enc_embd_dim, self.dec_embd_dim)
    ...
    def generate_context_embeddings(
        ...
        # shc: add positional embedding and task embedding to the entire output of encoder
        context_embeddings = []
        for task, info in input_info["tasks"].items():
            if self.task_embeddings is not None and task in self.task_embeddings:
                ...
                # shc: when we do not need task embedding
                task_emb = torch.zeros(
                    (bs, info["num_tokens"], self.dec_embd_dim), device=device
                )
            ...
            if info["has_3d_posemb"]:
                pos_embd = self.pos_embd_spatial.repeat(
                    ...
                    self.pos_embd_temporal, self.grid_size[1] * self.grid_size[2], dim=1
                )  # 1,nt*nh*nw, d
                pos_embd = pos_embd.expand(bs, -1, -1)

                task_emb = task_emb + pos_embd
    ...
    def get_queries_and_context(
        ...
        # Generate queries, use context embedding from that task as query
        ...
        # todo: shc understand ids_keep and ids_restore,
    ...
    def forward(
        ...
        x = rearrange(
            ...
            ph=self.spatial_patch_size,
            pw=self.spatial_patch_size,
            c=self.num_channels,
        )
