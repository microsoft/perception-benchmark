...
from .tMultiMAE.tmultimae import tmultimae
from .tMultiMAE.tmultimae.criterion import (
    MaskedCrossEntropyLoss,
    MaskedL1Loss,
    MaskedMSELoss,
)
from .tMultiMAE.utils.task_balancing import (
    NoWeightingStrategy,
    UncertaintyWeightingStrategy,
)
from .tMultiMAE.tmultimae.input_adapters import PatchedInputAdapter, SemSegInputAdapter
from .tMultiMAE.tmultimae.output_adapters import SpatialTemporalOutputAdapter
from .tMultiMAE.utils import create_model
from .tMultiMAE.utils.data_constants import COCO_SEMSEG_NUM_CLASSES

# high priority
# todo: fix grad stride warning
# todo: add visualization

# low priority
# todo: check positional embedding in input/output adapters
...
DOMAIN_CONF = {
    "rgb": {
        ...
        "output_adapter": partial(SpatialTemporalOutputAdapter, num_channels=3),
        "loss": MaskedMSELoss,
    },
    "depth": {
        "channels": 1,
        "input_adapter": partial(PatchedInputAdapter, num_channels=1),
        "output_adapter": partial(SpatialTemporalOutputAdapter, num_channels=1),
        "loss": MaskedL1Loss,
    },
    "semseg": {
        "num_classes": 133,
        ...
        "output_adapter": partial(
            SpatialTemporalOutputAdapter, num_channels=COCO_SEMSEG_NUM_CLASSES
        ),
...
def make_model(model_config: dict):
    ...
    input_adapters = {
        domain: DOMAIN_CONF[domain]["input_adapter"](
            ...
            spatial_patch_size=model_config["spatial_patch_size"],
    ...
    output_adapters = {
        domain: DOMAIN_CONF[domain]["output_adapter"](
            temporal_patch_size=model_config["temporal_patch_size"],
            spatial_patch_size=model_config["spatial_patch_size"],
            dec_embd_dim=model_config["decoder_dim"],
...
@registry.register_lightningmodule(name="tMultiMAE")
class tMultiMAEPretrain(pl.LightningModule):
    def __init__(
        self,
        model_config: dict,
    ...
    def prepare_batch(self, batch: Dict[str, torch.Tensor]) -> None:
        ...
        if self.hparams.model_config["standardize_depth"] and "depth" in batch:
            ...
            trunc_depth = torch.sort(
                rearrange(batch["depth"], "b c t h w -> b (c t h w)"), dim=1
            )[0]
            trunc_depth = trunc_depth[
                :, int(0.1 * trunc_depth.shape[1]) : int(0.9 * trunc_depth.shape[1])
            ]
            batch["depth"] = (
                batch["depth"] - trunc_depth.mean(dim=1)[:, None, None, None, None]
            ) / torch.sqrt(trunc_depth.var(dim=1)[:, None, None, None, None] + 1e-6)
